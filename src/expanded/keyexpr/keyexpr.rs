...
use crate::{
    z_keyexpr_autocanonize, z_keyexpr_concat, z_keyexpr_includes, z_keyexpr_intersects,
    z_keyexpr_join,
};
...
#[prebindgen]
pub fn keyexpr_join(a: impl Into<KeyExpr> + Send + 'static, b: String) -> Result<KeyExpr, Error> {
    let a_ke = into_native(a.into())?;
...
#[prebindgen]
pub fn keyexpr_concat(a: impl Into<KeyExpr> + Send + 'static, b: String) -> Result<KeyExpr, Error> {
    let a_ke = into_native(a.into())?;
